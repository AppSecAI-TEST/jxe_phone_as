...
public class UploadWaybill {
	...
	private static UploadWaybill uploadActivity = new UploadWaybill();

	private UploadWaybill() {
	}

	public static UploadWaybill getUploadActivity() {
		return uploadActivity;
	}

	public void upload(final Context mContext, final int type,
		...
		Cursor mCursor = acceptinputdbwrapper.rawQueryRank(0);
		do {
			...
			if (!mCursor.moveToNext()) {
				...
				j1 = mCursor.getColumnIndex("consType");
		...
		if (!jsonarray.toString().equals("[]")) {
			...
			// 讲数据保存到SD卡
			String sdCardExist = Environment.getExternalStorageState(); // sd卡
			if (sdCardExist.equals(Environment.MEDIA_MOUNTED)) {
				...
				String newDate = (new SimpleDateFormat("yyyy-MM-dd"))
						.format(new Date());
				edit.putString("day", newDate);

				try {
					if (newDate.equals(oldDate)) {
						LogUtils.newFile(MyApp.PATH, oldDate + ".txt",
			...
			httpUtils.send(HttpMethod.POST, MyApp.mPathServerURL
					+ Conf.saveWaybill, params, new RequestCallBack<String>() {
				...
				public void onSuccess(ResponseInfo<String> arg0) {
					...
					I_SaveWaybill saveWaybill = new Gson().fromJson(result,
							I_SaveWaybill.class);
					if (saveWaybill.isSuccess()) {

						AcceptInputDBWrapper inputDBWrapper = AcceptInputDBWrapper
								.getInstance(mContext);
						childNoListDBWrapper childNoListDBWrapper2 = childNoListDBWrapper
								.getInstance(mContext);
						// 查询到未上传的
						Cursor mCursor = inputDBWrapper.rawQueryRank(0);
						do {
							if (mCursor.moveToNext()) {
								...
								// 根据单号查询是否有子单号
								Cursor cursor = childNoListDBWrapper2
										.rawQueryRank(waybillNo, 0);
								do {
									if (cursor.moveToNext()) {
										childNoListDBWrapper2.rawUpdateRank(
												waybillNo, 1);
									} else {
										cursor.close();
										break;
								...
								mCursor.close();
								Toast.makeText(mContext, "上传成功", 1).show();
								if (type == 1) {
						...
						Toast.makeText(mContext, saveWaybill.getError(), 0)
								.show();
					}
